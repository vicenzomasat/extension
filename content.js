(function() {
  ...
  function injectScript(fn, ...args){
    try {
      ...
      const ser=args.map(a=>{try{return JSON.stringify(a);}catch{ return 'null';}}).join(',');
      s.textContent='('+fn.toString()+')('+ser+');';
      (document.head||document.documentElement).prepend(s); s.remove();
    } catch(e){}
  }
  ...
  function pageMain(settings, WL, BL){
    ...
    function cloneFunctionSignature(original, wrapper){
      try { wrapper.toString = originalToString.bind(original); } catch(_){ }
    ...
    if (applyProtection && settings.spoofUserAgent){
      const realNav = window.navigator; // can't change prototype of native Navigator
      ...
      const navProxy = new Proxy(realNav, {
        get(target, prop, receiver){
          switch(prop){
            case 'userAgent': return personaUA();
            case 'platform': return personaPlatform();
            case 'language': return personaLang();
            case 'languages': return personaLangs().slice();
      ...
      try {
        Object.defineProperty(window,'navigator',{get: cloneFunctionSignature(function(){}, function(){ return navProxy; }), configurable:true});
      }catch(_){ /* ignore */ }
    }
    ...
    // --- Canvas minimal noise (<0.01%) ---
    if (applyProtection && settings.spoofCanvas){
      const origGetContext = HTMLCanvasElement.prototype.getContext; // unavoidable single prototype wrap
          ...
          return new Proxy(ctx, {
            get(target, prop){
              if (prop==='getImageData'){
                ...
                return cloneFunctionSignature(orig, function(x,y,w,h){ const data = orig(x,y,w,h); try { const total = data.data.length/4; const changes = Math.max(1, Math.floor(total*0.00005)); for(let i=0;i<changes;i++){ const p = (Math.random()*total)|0; const idx=p*4; // adjust only one channel by +-1
                  data.data[idx] = (data.data[idx] + (Math.random()<0.5?-1:1)) & 0xFF; } }catch(_){ } return data; });
              }
              if (prop==='toDataURL'){
                const orig = target.toDataURL.bind(target);
                return cloneFunctionSignature(orig, function(...a){ try { // force a tiny pre-read to introduce noise path
                  const w = target.canvas.width; const h = target.canvas.height; if (w*h>0 && w*h<500000){ const sample = target.getImageData(0,0,Math.min(16,w), Math.min(16,h)); sample.data[0] ^= 0; }
                }catch(_){ } return orig(...a); });
              }
              if (prop==='toBlob' && typeof target.toBlob==='function'){
                const orig = target.toBlob.bind(target);
                return cloneFunctionSignature(orig, function(cb,type,qual){ return orig(function(blob){ cb(blob); }, type, qual); });
              }
    ...
    if (applyProtection && settings.spoofWebGL){
      ...
      function patchContext(gl){
        ...
        const personaVals = () => persona && persona.webgl ? persona.webgl : { vendor:'Google Inc.', renderer:'ANGLE (Intel, Intel(R) UHD Graphics)' };
        function wrappedGetParameter(p){
          const vals = personaVals();
            if (p===unmaskedVendor || p===vendorConst){ if (Math.random()<0.6) return vals.vendor; }
            if (p===unmaskedRenderer || p===rendererConst){ if (Math.random()<0.6) return vals.renderer; }
          return realGetParameter(p);
        }
        cloneFunctionSignature(realGetParameter, wrappedGetParameter);
        const handler = {
          get(t,prop){ if (prop==='getParameter') return wrappedGetParameter; return Reflect.get(t,prop); }
        };
        const prox = new Proxy(gl, handler);
        overrideParams.set(gl, prox); return prox;
      }
      const origGetContext2 = HTMLCanvasElement.prototype.getContext;
      function wrapped(type,...rest){ const ctx = origGetContext2.apply(this,[type,...rest]); if(!ctx) return ctx; if (/webgl2?/i.test(type)) return patchContext(ctx); return ctx; }
    ...
    // --- Fingerprinting detection (lightweight) ---
    if (applyProtection && settings.detectFingerprinting){
      let fpCount = 0;
      function report(method, property, extra){ try { window.postMessage({ type:'PRIVACY_SHIELD_DETECTION', method, property, value:extra, count: ++fpCount }, '*'); }catch(_){ } }
      // Access patterns
      const nav = navigator; let navAccess=0; ['hardwareConcurrency','platform','languages','language','userAgent'].forEach(k=>{ try { Object.defineProperty(Navigator.prototype, '__ps_'+k, { get: function(){ return nav[k]; }, configurable:true }); }catch(_){ } });
      const navProxy = window.navigator; // already maybe proxied
      ...
      trackProps.forEach(prop=>{
        try {
          ...
          if (origDesc && origDesc.get){
            ...
            function wrapped(){ navAccess++; if (navAccess>5) report('navigator_fingerprint', prop); return originalGetter.call(this); }
            cloneFunctionSignature(originalGetter, wrapped);
            Object.defineProperty(Navigator.prototype, prop, { get: wrapped, configurable:true });
          }
        }catch(_){ }
      });
    }
